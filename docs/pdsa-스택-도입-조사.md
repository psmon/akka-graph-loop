...
| B1 | Kùzu 명시적 트랜잭션 | next(검증 선행) | N1 의 더 강한 대안. C API 노출 여부 **미검증** |
| B2 | Polly / Microsoft.Extensions.Http.Resilience | reject | N3(25줄)로 충분. AOT 39MB 바이너리에 DI 스택 추가는 과함 |
| B3 | OpenTelemetry | reject(CLI) / later(Viewer) | 수명 300~500ms 프로세스에 익스포터 부적합. N2 가 대체 |
...
| CLI 콜드 스타트(LLM 미사용, `status`) | **424 / 422 / 496 ms** | 3회 반복 측정 |
| LLM 왕복(`check`, 최소 프롬프트) | **2,346 ms** | `pdsa check` 출력 |

이 기준선이 판정을 좌우한다: 프로세스 수명이 **0.4~3초**인 CLI 에서
...
프로세스당 LLM 호출 1회 + 그래프 쓰기 몇 회로 끝나는 **0.4~3초짜리 단발 실행**이고,
...
수명 300~500ms 프로세스는 익스포터 플러시 시간이 작업 시간과 맞먹는다.
