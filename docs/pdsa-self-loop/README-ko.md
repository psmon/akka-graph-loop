...
이 프로젝트는 PDSA 도구를 *만들기만* 한 게 아니다. **그 도구로 만들어졌다.** 2026-08-26 이후 22번의
사이클이 `pdsa` CLI 자신의 Plan → Do → Study → Act 루프를 돌았고, 모든 단계가 지금도 그래프에 남아 있다.
(모든 커밋이 사이클을 탄 것은 아니다. 어떤 것이 안 탔고 왜인지는 아래 막간에 따로 적었다.)
