...
- **Study** — **`partial`.** All 73 tests passed and the `partial → reinforce → met` flow was confirmed,
  but the eval hit rate was 1/2 (50%) and the "90% agreement across 10 representative cycles" check was never done.
- **Act** — Structure the Study input and judging criteria to raise verdict reproducibility. **Needs reinforcement (→ #5).**
...
- **Study** — **`partial`.** The design settled config UX, the auth abstraction, five backward-compat
...
- **Study** — **`partial`.** 73→91 tests all green, keyless localhost load and remote no-key blocking confirmed —
...
- **Study** — **`partial`.** 100 tests and zero warnings; the E2E confirmed blocking without opt-in (exit 3)
...
- **Study** — **`met`.** 102→129 tests green, zero warnings. Token refresh, persist, no effect on ApiKey/None,
...
- **Study** — **`partial`.** 129→141 tests, 12 new, real CLI E2E for create/protect/force-overwrite, zero warnings —
...
- **Study** — **`met`.** 141→163 tests green, zero warnings. All five precedence levels reproduce in isolated
...
- **Study** — **`unmet`.** The implementation, tests and config smoke all checked out, but **the expected
...
- **Study** — **`met`.** Compared four candidates on cost, performance/fit, integration, .NET AOT, official
...
- **Study** — **`met`.** 175→189 tests green, zero warnings. A real `claude -p` round trip (**7,411ms**) returned
...
- **Study** — **`partial`.** Concurrent `plan --project` on two projects showed zero body contamination, separate
...
- **Study** — **`met`.** All four phases recorded in both projects, **grep for own and other project names across
...
- **Study** — **`met`.** The AOT single `pdsa.exe` (40MB) started the server with no separate dll, all three routes
...
- **Study** — **`met`.** Precedence pinned by a 5-case Theory and an E2E (env=1s overriding config=90s);
...
- **Study** — **`met`.** All seven commands emit valid camelCase JSON, recall→plan injection confirmed,
...
- **Do** — Three axes: codebase measurement, failure injection, API existence verification. **`IPdsaEngine` is
  ...
  orphan cycle (2/2).
- **Study** — **`met`.** All 20 candidates got evidence, a four-lens evaluation, a verification method and a
...
- **Study** — **`met`.** 20 injected failures produced **0 orphans** (baseline 100%), the retry policy is pinned by
...
- **Study** — **`met`.** Correct output across the real 21-cycle DB, both directions of REINFORCES, boundary cases
